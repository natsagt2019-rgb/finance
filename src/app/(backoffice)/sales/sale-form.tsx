...
export function SaleForm({ accounts }: { accounts: AccOpt[] }) {
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    start(async () => {
      ...
      if (res.ok) {
        setMsg({ ok: true, text: "✓ Борлуулалт бүртгэгдэж, авлага журналд бичигдлээ." });
  ...
  return (
    <form onSubmit={onSubmit} className="rounded-2xl border border-zinc-200 bg-white p-5">
      <h2 className="text-sm font-semibold text-zinc-800">Шинэ борлуулалт</h2>
      <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
        <div>
          <label className={lbl}>Огноо</label>
          <input type="date" name="sale_date" required className={inputCls} />
        </div>
        <div>
          <label className={lbl}>Баримт №</label>
          <input name="doc_no" placeholder="БО-001" className={inputCls} />
        </div>
        <div>
          <label className={lbl}>Үйлчлүүлэгч</label>
          <input name="partner_name" placeholder="Үйлчлүүлэгчийн нэр" className={inputCls} />
        </div>
        <div className="sm:col-span-2">
          <label className={lbl}>Орлогын данс (Кт)</label>
          <input list="sale-acc" name="revenue_code" required placeholder="код эсвэл нэр" className={inputCls} />
          <datalist id="sale-acc">
            {accounts.map((a) => (
      ...
      <p className="mt-2 text-xs text-zinc-400">
        Бичилт: Дт 120101 авлага (нийт) / Кт сонгосон орлого (цэвэр) + Кт 310601 НӨАТ. → /receivables, харилцагчийн тооцоонд тусна.
      </p>
    </form>
  );
}
